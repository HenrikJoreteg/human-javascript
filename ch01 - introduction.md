...
The challenge with those frameworks, from where I sit (which happens to be in a plane 37,000ft over the Atlantic at the moment), is that there's a lot of emphasis on trying to share code and logic between the client and the server when client/server really should be performing fundamentally different roles. Servers are for data, clients are for presentation. To me, this is just about the basic principle of seperation of concerns.
